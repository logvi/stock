    ...
    def diverMACD(self, qs, MACDField):
        ...
        while i<len(qs):
            ...
            if(qs[i]['low']<min_low):
                min_low = qs[i]['low']
            if(len(qs[i-3:i])!=0):
                x1 = qs[i][MACDField]['signal']['ema']
                x = qs[i-1][MACDField]['signal']['ema']
                x3 = qs[i-2][MACDField]['signal']['ema']
                x4 = qs[i-3][MACDField]['signal']['ema']
                ...
                if (x1>x and x<x3<x4):
                    ...
                    x_macd = qs[i-1][MACDField]['signal']['ema']
                    x_price = min_low
